...
def cart_view(request):
    try:
        ...
        if request.user.is_authenticated:
            ...
            logger.info(
                f"Найдено элементов корзины:"
                f" {cart_items.count()}, IDs: {[item.id for item in cart_items]}"
            )
...
def add_to_cart(request, product_id):
    ...
    try:
        ...
        if request.user.is_authenticated:
            logger.info(
                f"Adding to cart for user {request.user.email}:"
                f" product {product_id}, size {size}, qty {quantity}"
            )
            ...
            if not created:
                ...
                item.save()
            logger.debug(
                f"Cart item {'created' if created else 'updated'}: ID {item.id}"
            )
        else:
            logger.info(
                f"Adding to guest cart: "
                f"product {product_id}, size {size}, qty {quantity}"
            )
            session_cart = request.session.get("cart", [])
            for entry in session_cart:
                ...
                session_cart.append(
                    {"product_id": product_id, "size": size, "quantity": quantity}
                )
        ...
        return JsonResponse(
            {
                ...
                "total_items": (
                    ...
                    if request.user.is_authenticated
                    else sum(entry["quantity"] for entry in session_cart)
                ),
            }
        )
    except Product.DoesNotExist:
...
def create_order_guest(request):
    ...
    return render(
        request, "orders/guest_checkout.html", {"items": items, "total": total}
    )
...
def update_cart(request, item_id):
    if request.method == "POST":
        try:
            ...
            item.save()
            print(
                f"[update_cart] Updated item "
                f"{item_id} to qty={quantity} for user={request.user}"
            )
            return JsonResponse({"success": True})
        except Exception as e:
            import traceback
