...
def profile_view(request):
    try:
        ...
        logger.info(
            f"Профиль для {user.email}: создан={created}, аватар={profile.avatar.url if profile.avatar else 'нет'}"
        )
        form = ProfileForm(
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                form.save()
                logger.info(
                    f"Аватар сохранен: {profile.avatar.url if profile.avatar else 'нет'}"
                )
...
def toggle_favorite(request, product_id):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"success": True, "action": action, "likes_count": likes_count}
            )
